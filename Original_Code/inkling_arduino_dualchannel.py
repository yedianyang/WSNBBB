...
def move_the_board():
    ...
    command(serClearCore, "X10,Y10/r/n")
    ...
    while (1):
        try:
            ...
            # if serial_reading != 0:
            if(len(serial_reading) == 23):
                if serial_reading[0] == 62:    # DEC number of ASCII Char ">"
                    if serial_reading[-2] == 60:
                        
                        decoded_reading = serial_reading.decode("ascii")

                        x_platform_current = round(
                            float(decoded_reading.split(':')[1]), 2)  # * 75.1 / 4095
                        # * 75.1 / 4095
                        y_platform_current = round(
                            float(decoded_reading.split(':')[3].split('<')[0]), 2)

                        # print(datetime.now(), 'thread2 x_platform_current:%.2f\ty_platform_current:%.2f\tpressed:%d' % (
                        #     x_platform_current, y_platform_current, pressed))
                        ...
                        print(datetime.now(), 'thread2 x_plfm_current: %d\tx_move%d\tx_plfm_moveTo%d\tpressed%d'%(x_platform_current, x_move, x_platform_moveTo,pressed))
                        print(datetime.now(), 'thread2 y_plfm_current: %d\ty_move%d\ty_plfm_moveTo%d\tpressed%d'%(y_platform_current, y_move, y_platform_moveTo,pressed))
                        ...
                        if ((x_platform_moveTo > 1 and x_platform_moveTo <= 500)):
                            if (((y_platform_moveTo > 1 and y_platform_moveTo < 500))):
                                command_tmp = \
                                    "X" + str(round(float(x_platform_moveTo), 1)) + \
                                    ",Y" + \
                                    str(round(float(y_platform_moveTo), 1)) + "\r\n"
    ...
    while (1):
        try:
            with lock2:
                ...
                print(datetime.now(), 'thread3 x_platform_current:%.2f\ty_platform_current:%.2f\tpressed:%d' % (
                    x_platform_current, y_platform_current, pressed))
