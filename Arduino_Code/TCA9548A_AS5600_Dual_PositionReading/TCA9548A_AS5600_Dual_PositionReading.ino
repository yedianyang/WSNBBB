...
void loop() {
  ...
  if (millis() - lastTime >= 5) {
    ...
    TCA9548(I2CA);
    X = stpA.getCumulativePosition() * 75.0 / 4096;
    // Switch the address the I2CB
    TCA9548(I2CB);
    Y = stpB.getCumulativePosition() * 75.0 / 4096;
...
void printPadded(float value, char axis) {
  // 计算整数和小数部分
  int intPart = (int)value;
  int decimalPart = abs((int)((value - intPart) * 100));

  // 按照格式输出
  Serial.print(axis);
  if (value >= 0 && value < 10) Serial.print("000");
  else if (value >= 10 && value < 100) Serial.print("00");
  else if (value >= 100) Serial.print("0");
  else if ( value < -100) Serial.print("-");
  else if (value <= -10 ) Serial.print("-0");
  else if (value < 0 && value > -10) Serial.print("-00");
  
  if(value >= 0) Serial.print(intPart);
  if(value < 0) Serial.print(abs(intPart));
  Serial.print(".");
